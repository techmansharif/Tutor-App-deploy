...
def process_latex_file(latex_file_path: str, image_folder: str, subject_name: str, topic_name: str):
    ...
    Updates existing Explain entries instead of deleting them.
    ...
    try:
        ...
        for part_title, part_content in parts:
            ...
            # Check if Explain entry already exists
            existing_explain = db.query(Explain).filter(Explain.subtopic_id == subtopic.id).first()
            
            if existing_explain:
                # Update existing Explain entry with new chunks
                existing_explain.chunks = chunks
                db.commit()
                print(f"Updated existing Explain entry for subtopic: {subtopic_name} with {len(chunks)} chunks")
            else:
                ...
                db.commit()
                db.refresh(explain)
                print(f"Created new Explain entry for subtopic: {subtopic_name} with {len(chunks)} chunks")
