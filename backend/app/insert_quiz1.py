...
def insert_data_to_database(subject_name, topic_name, questions_data, db):
    ...
    for subtopic_name, questions in subtopics_dict.items():
        ...
        if duplicate_questions_count > 0:
            print(f"⚠️  Skipped {duplicate_questions_count} duplicate questions in subtopic: {subtopic_name}")
...
def process_folders_and_files(base_directory="data"):
    ...
    try:
        ...
        for subject_folder in os.listdir(base_directory):
            ...
            for file_name in os.listdir(subject_path):
                if file_name.endswith('.csv'):
                    ...
                    # Check if topic exists before processing
                    ...
                    if not topic_exists:
                        topic_after = db.query(Topic).filter(
                            Topic.name == topic_name, 
                            Topic.subject_id == subject.id
                        ).first()
                        if topic_after:
        ...
        print(f"✅ Total new topics created: {total_topics_created}")
...
def main():
    ...
    print("⚠️  Note: Subjects must exist in database before running this script")
    print("✅ Topics and Subtopics will be created automatically if they don't exist")
    print("=" * 80)
    
    try:
        process_folders_and_files("D:\Tutor\Tutor-App-deploy/backend/app\quiz1_initial")  # Change "data" to your base directory name
    except Exception as e:
        print(f"\n💥 FATAL ERROR: {str(e)}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("📊 Check the summary above for details on what was created and inserted.")
    else:
        print("\n❌ Process failed. Check the errors above and try again.")
    
    # These lists are now less relevant since we create missing items
    if topic_not_exist:
        print(f"\n📝 Topics that were missing (now created): {topic_not_exist}")
    if subtopic_not_exist:
        print(f"\n📝 Subtopics that were missing (now created): {subtopic_not_exist}")