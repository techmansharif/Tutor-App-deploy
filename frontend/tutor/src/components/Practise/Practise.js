...
const PracticeQuiz = ({ user, API_BASE_URL }) => {
  ...
  const fetchPracticeQuestion = useCallback(
  ...
  const handleCompletePractice = () => {
    if (subject && topic && subtopic) {
      ...
      console.error('Missing URL parameters for quiz redirect:', { subject, topic, subtopic });
      navigate('/select');
    }
  };
    ...
    <div className="practice-quiz-container">
      <div className="quiz-header">
        <div className="quiz-title-section">
          <h2>
            <i className="bi bi-journal-bookmark-fill"></i> PRACTICE
          </h2>
          <h3>{subject}</h3>
          <h3>{topic}</h3>
          <h3>{subtopic}</h3>
          ...
          <div className="modal-content">
            <div className="feedback incorrect">
              ...
              <p style={{ marginLeft: '20px', paddingLeft: '10px', textAlign: 'left' }}>
                <strong>Correct Answer- </strong> {currentQuestion.correct_option.toUpperCase()}:{' '}
                <MathText>{currentQuestion[`option_${currentQuestion.correct_option}`]}</MathText>
              </p>
